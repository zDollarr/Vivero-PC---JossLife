...
interface SidebarProps {
  onReport?: () => void;
  isOnline: boolean; // NUEVA PROP
}

export const Sidebar = ({ onReport, isOnline }: SidebarProps) => {
  ...
  return (
    ...
    <aside className="w-72 bg-gradient-to-b from-green-900 to-green-950 text-white flex flex-col shadow-2xl z-10 relative h-screen">
      ...
      <div className="p-8 border-b border-green-800/50">
        ...
        <p className="text-green-400 text-xs font-medium uppercase tracking-widest ml-1">Panel de control - Admin</p>
      </div>
      
      <nav className="flex-1 p-6 space-y-4">
        <button className="block w-full py-4 px-6 bg-green-800/60 rounded-2xl text-white font-bold tracking-[0.15em] text-center shadow-lg border border-green-700/30 hover:bg-green-700 transition-all cursor-default">
          INVENTARIO
        </button>

        <button 
          onClick={onReport}
          className="block w-full py-4 px-6 bg-green-900/20 rounded-2xl text-green-100/70 font-bold tracking-[0.15em] text-center border border-transparent hover:bg-green-800/40 hover:text-white transition-all"
        >
          GENERAR REPORTE
        </button>
      </nav>
          ...
          <div className="flex items-center gap-3 p-2 rounded-xl flex-1 cursor-default">
             ...
             <div className="flex-col">
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  <span className={`w-2 h-2 rounded-full ${isOnline ? 'bg-green-400 animate-pulse' : 'bg-orange-500'}`}></span>
                  <p className={`text-[10px] font-medium ${isOnline ? 'text-green-300' : 'text-orange-300'}`}>
         ...
         <div className="fixed inset-0 z-[150] flex items-center justify-center animate-fadeIn">
            ...
            <div className="bg-white rounded-3xl p-8 max-w-sm w-full relative z-[160] text-center shadow-2xl">
               ...
               <div className="flex gap-3">
                  <button onClick={() => setShowLogoutConfirm(false)} className="flex-1 py-3 bg-gray-100 text-gray-700 font-bold rounded-xl hover:bg-gray-200 transition-colors">Cancelar</button>
                  <button onClick={handleLogoutConfirm} className="flex-1 py-3 bg-green-600 text-white font-bold rounded-xl hover:bg-green-700 transition-colors shadow-lg">Salir</button>
               </div>
            </div>
         </div>
    )}
